...
export class CSLoadingService {
  private readonly logger = inject(LoggerService);
  private readonly loadingClassName = 'cs-loading-active';
  private readonly _auto$: BehaviorSubject<boolean> =
    new BehaviorSubject<boolean>(true);
  private readonly _mode$: BehaviorSubject<'determinate' | 'indeterminate'> =
    new BehaviorSubject<'determinate' | 'indeterminate'>('indeterminate');
  private readonly _progress$: BehaviorSubject<number | null> =
    new BehaviorSubject<number | null>(0);
  private readonly _show$: BehaviorSubject<boolean> =
    new BehaviorSubject<boolean>(false);
  private readonly _urlMap: Map<string, boolean> = new Map<string, boolean>();
  ...
  setProgress(value: number): void {
    if (value < 0 || value > 100) {
      this.logger.error('Progress value must be between 0 and 100!');
      return;
    }
  ...
  _setLoadingStatus(status: boolean, url: string): void {
    ...
    if (!url) {
      this.logger.error('The request URL must be provided!');
      return;
    }
  ...
  private _syncLoadingClass(): void {
    if (typeof document === 'undefined' || !document.body) {
      return;
    }

    const hasPendingRequests = this._urlMap.size > 0;
    document.body.classList.toggle(this.loadingClassName, hasPendingRequests);
  }
}
