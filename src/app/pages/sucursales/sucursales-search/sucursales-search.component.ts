...
import { SucursalListItem } from '../models/sucursal.model';

@Component({
  selector: 'app-sucursales-search',
  standalone: true,
  templateUrl: './sucursales-search.component.html',
...
export class SucursalesSearchComponent implements OnInit {
  ...
  readonly items = signal<SucursalListItem[]>([]);
  ...
  readonly breadcrumbs: BreadcrumbItem[] = [
    ...
    { label: 'breadcrumbs.sucursales', isActive: true },
  ];

  readonly displayedColumns = ['nombre', 'numeroLicencia', 'direccionFisica', 'gerenteNombre'];
  ...
  search(): void {
    ...
    const { nombre, numeroLicencia, direccionFisica } = this.searchForm.value;
    this.service
      .search({
      ...
      .subscribe({
        ...
        error: () => {
          this.loading.set(false);
          this.notifier.showError(this.transloco.translate('sucursales.search.search-error'));
        },
      });
  }
  ...
  exportar(): void {
    const t = (key: string) => this.transloco.translate(key, {}, 'sucursales');
    const headers = [
      t('table.nombre'),
      t('table.direccionFisica'),
      t('table.numeroLicencia'),
      t('table.gerenteNombre'),
    ];
    const { nombre, numeroLicencia, direccionFisica } = this.searchForm.value;
    this.service
      .exportar(headers, {
      ...
      .subscribe({
        next: blob => {
          ...
          a.href = url;
          a.download = this.transloco.translate('sucursales.search.export-filename');
          a.click();
          URL.revokeObjectURL(url);
        },
        error: () => this.notifier.showError(this.transloco.translate('sucursales.search.search-error')),
      });
  }
}
