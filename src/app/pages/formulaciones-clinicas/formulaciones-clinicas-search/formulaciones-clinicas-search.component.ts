...
import { FormulacionClinicaListItem } from '../models/formulacion-clinica.model';

@Component({
  selector: 'app-formulaciones-clinicas-search',
  standalone: true,
  templateUrl: './formulaciones-clinicas-search.component.html',
...
export class FormulacionesClinicasSearchComponent implements OnInit {
  ...
  readonly items = signal<FormulacionClinicaListItem[]>([]);
  readonly totalCount = signal(0);
  readonly pageIndex = signal(0);
  readonly pageSize = signal(10);

  readonly searchForm = this.fb.group({ productoNombre: [''], ingredienteNombre: [''] });

  readonly breadcrumbs: BreadcrumbItem[] = [
    { label: 'breadcrumbs.home', route: '/dashboard' },
    { label: 'breadcrumbs.catalogos' },
    { label: 'breadcrumbs.formulaciones-clinicas', isActive: true },
  ];
  ...
  search(): void {
    ...
    this.service
      ...
      .subscribe({
        ...
        error: () => {
          this.loading.set(false);
          this.notifier.showError(this.transloco.translate('formulacionesClinicas.search.search-error'));
        },
      });
  }
